...
createSurfaceMap <- function(df, var_name){
  # additional: parameters
  # model = "Sph" Try "Exp" (exponential) or "Gau" (Gaussian) if the variogram fit looks off
  MODEL <- "Exp"
  # length.out = 200 Increase for a finer grid (slower), decrease for speed
  ...
  bbox_ext  <- 0.1
  lon_range <- seq(bbox(df_sp)[1,1] - bbox_ext, bbox(df_sp)[1,2] + bbox_ext, length.out = 200)
  lat_range <- seq(bbox(df_sp)[2,1] - bbox_ext, bbox(df_sp)[2,2] + bbox_ext, length.out = 200)
  ...
  krig_result <- krige(mean_value ~ 1, locations = df_sp, newdata = grid, model = vgm_fit)
...
# --- Hardcoded legend limits ---
legend_min <- 20   # <-- set your min here
legend_max <- 30   # <-- set your max here
...
createSurfaceMap(cruise_df, "sea_water_temperature")
