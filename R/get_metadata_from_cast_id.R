get_metadata_from_cast_id <- function(cast_id){
    ...
    cast_id <- sub("^[^_]+_[^_]+_", "", params$cast_id)
    # extract station_id from cast_id by skipping first (1-3) letters and next (4-5) digits and optional _
    station_id <- sub("^[A-Za-z]{1,3}[0-9]{4,5}_?", "", cast_id)
    # drop stn, sta from station_id like `stn2`
    station_id <- sub("(?i)(stn|sta)_?", "", station_id)

    # get cruise ID from first part of cast_id by _
